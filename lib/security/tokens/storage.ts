...
export const TOKEN_EXPIRY = {
  STORAGE_ACCESS: 3600, // 1 hour - for accessing storage resources
...
export function signStorageToken(
  payload: Omit<StorageTokenPayload, 'exp' | 'iat'>,
  expirySeconds: number = TOKEN_EXPIRY.STORAGE_ACCESS
): string {
  const secret = secretOrThrow();
  const now = Math.floor(Date.now() / 1000);
  
  const fullPayload: StorageTokenPayload = {
    ...payload,
    iat: now,
...
export function verifyStorageToken(token: string): StorageTokenPayload | null {
  try {
    ...
    const payload = JSON.parse(b64urlDecode(p).toString("utf8")) as StorageTokenPayload;
    
    // Validate all required fields
    if (!payload?.sub || !payload?.bucket || !payload?.key || !payload?.exp || !payload?.iat) {
      return null;
    }
    
    if (payload.bucket !== "book" && payload.bucket !== "video" && payload.bucket !== "proof-payment") {
      return null;
    }
