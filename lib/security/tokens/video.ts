...
export const TOKEN_EXPIRY = {
  VIDEO_PLAY: 3600, // 1 hour - enough for watching a video
...
export type VideoPlayTokenPayload = {
  sub: string; // user id (for auditing; the token itself is the gate)
  videoId: string;
  bucket: "video";
  key: string;
...
function secretOrThrow(): string {
  return env.security.contentTokenSecret();
}

export function signVideoToken(
  payload: Omit<VideoPlayTokenPayload, 'exp' | 'iat'>,
  expirySeconds: number = TOKEN_EXPIRY.VIDEO_PLAY
): string {
  const secret = secretOrThrow();
  const now = Math.floor(Date.now() / 1000);
  
  const fullPayload: VideoPlayTokenPayload = {
    ...payload,
    iat: now,
...
export function verifyVideoToken(token: string): VideoPlayTokenPayload | null {
  try {
    const secret = secretOrThrow();
    ...
    const payload = JSON.parse(b64urlDecode(p).toString("utf8")) as VideoPlayTokenPayload;
    
    // Validate all required fields
    if (!payload?.sub || !payload?.videoId || !payload?.key || !payload?.bucket || !payload?.exp || !payload?.iat) {
      return null;
    }
    
    if (payload.bucket !== "video") return null;
